...
export interface AmericanMethodResult {
...
/**
 * Main American method calculation function
 */
export function calculateAmericanMethod(data: CashFlowFormData): AmericanMethodResult {
  ...
  data.gracePeriod.forEach(gp => {
    gracePeriodMap.set(gp.period, gp.type === GracePeriodType.total ? 'T' : gp.type === GracePeriodType.partial ? 'P' : 'S');
  });
  for (let i = 1; i <= totalPeriods; i++) {
  ...
  for (let i = 1; i <= totalPeriods; i++) {
    ...
    const coupon = isTotalGrace ? new Decimal(0) : remainingBond.mul(effectivePeriodRate);

    // Calculate amortization and quota
    ...
    if (isTotalGrace) {
      ...
      // Standard period: pay interest only, except in last period
      quota = coupon;
      if (isLastPeriod) {
        // In the last period, amortize the entire principal
        amortization = remainingBond;
        quota = quota.plus(amortization);
      }
    }
 ...
 */
export function testAmericanMethod(): AmericanMethodResult {
  ...
  return calculateAmericanMethod(testData);
}

/**
 * Modified calculateGermanBondSchedule to use American method
 */
export function calculateAmericanBondSchedule(input: PaymentPlanInput): PaymentRow[] {
  ...
  // Calculate using the American method
